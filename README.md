...
- **Preview before packaging**: inspect a selected item in a hierarchical tree, list, table, or large-icon view; exclude individual files and folders before creating an archive.
...
The GitHub Actions workflow builds Windows `x64`, `x86`, and `arm64` release artifacts with PyInstaller. Releases are built with `--noconsole` so a console window is not created by default.
...
- **打包前预览**：以目录树、列表、表格或大图标查看选中项；可在创建备份前排除具体文件或文件夹。
...
GitHub Actions 会通过 PyInstaller 构建 Windows `x64`、`x86` 和 `arm64` 三种发布产物，并采用 `--noconsole`，因此默认不会出现控制台窗口。
